...
class Board():
    ...
    def display_cardlist(self):
        ...
        window_label = tk.Label(window2, text = "window2")
        ...
        w2 = tk.Canvas(window2, height=self.CANVAS_H, width=self.CANVAS_W, bg="light blue")
        xloc = self.X_OFFSET
        yloc = self.Y_OFFSET
    ...
    def display_6hands(self):
        ...
        w = tk.Canvas(window3, height=self.CANVAS_H, width=self.CANVAS_W, bg="light blue")
        ...
        for i in range(6):
            ...
            # printing row labels
            tk.Label(window3, text="H1", font=font_size, bg=color, width=label_width).place(x=xloc, y=yloc)
            tk.Label(window3, text="H2", font=font_size, bg=color, width = label_width).place(x=xloc, y=yloc+1*line_height)
            tk.Label(window3, text="H3", font=font_size, bg=color, width = label_width).place(x=xloc, y=yloc+2*line_height)
            tk.Label(window3, text="H4", font=font_size, bg=color, width = label_width).place(x=xloc, y=yloc+3*line_height)
            tk.Label(window3, text="H5", font=font_size, bg=color, width = label_width).place(x=xloc, y=yloc+4*line_height)
            tk.Label(window3, text="H6", font=font_size, bg=color, width = label_width).place(x=xloc, y=yloc+5*line_height)
            tk.Label(window3, text="Tot", font=font_size, bg=color, width = label_width).place(x=xloc, y=yloc+6*line_height)
            for j in range(6):
                ...
                tk.Label(window3, text=player, font=("Arial 10 bold"), bg=color, width = label_width, anchor="e").place(x=xloc+2, y=yloc-1*line_height)
                # displaying column of points for player j on rows 2-7
                tk.Label(window3, text=str(player_win_points[i][j][1]), font=font_size, bg=color, width = label_width, anchor="e").place(x=xloc, y=yloc+0*line_height)
                tk.Label(window3, text=str(player_win_points[i][j][2]), font=font_size, bg=color, width = label_width, anchor="e").place(x=xloc, y=yloc+1*line_height)
                tk.Label(window3, text=str(player_win_points[i][j][3]), font=font_size, bg=color, width = label_width, anchor="e").place(x=xloc, y=yloc+2*line_height)
                tk.Label(window3, text=str(player_win_points[i][j][4]), font=font_size, bg=color, width = label_width, anchor="e").place(x=xloc, y=yloc+3*line_height)
                tk.Label(window3, text=str(player_win_points[i][j][5]), font=font_size, bg=color, width = label_width, anchor="e").place(x=xloc, y=yloc+4*line_height)
                tk.Label(window3, text=str(player_win_points[i][j][6]), font=font_size, bg=color, width = label_width, anchor="e").place(x=xloc, y=yloc+5*line_height)
                tk.Label(window3, text=str(player_win_points[i][j][0]), font=font_size, bg=color, width = label_width, anchor="e").place(x=xloc, y=yloc+6*line_height)

            # displaying total points on row 8
            tk.Label(window3, text=str(total_points),
...
board4 = Board()
playsixhands = PlaySixHands25(Deck2.deal())
