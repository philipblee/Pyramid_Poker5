""" RunPyramidPoker4 - going to try to expand points from 3 cards to 5 cards
    ...
    Trying to use as much of display_points as possible, replacing dispay_score
"""
# todo expand points
...
top = tk.Tk()
X_GAP = 80
w = tk.Canvas(top, height=900, width=13.5 * X_GAP, background="pink", relief="raised")
this_hand = PokerHand()
if MODE == "Batch":
    overall_start_time = time.time()
    NUMBER_OF_CARDS = 25
...
def solve_best25_hand(*args):
    ...
    for i in range(1):
        ...
        display_points(score_array, x, y)
...
def show_next_hand(*args):
    ...
    print ("display_next_hand", twentyfive_cards)
...
def show_best25_hand(*args):
    ...
    player_total_score = player_score[0]  # this statement breaks on invalid hand
...
def show_player_score():
    ...
    if valid_hand == False:
        ...
        display_points(player_hand_score, x, y + 25)
        print ("Player Hand Invalid, player_score set to -9999, restore_position")
        player_hand_score = [0,0,0,0,0,0,0,0]
        # show_twentyfive_cards()
