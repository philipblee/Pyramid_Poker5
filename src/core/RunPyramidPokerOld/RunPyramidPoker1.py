...
import time
import src.core.global_variables
import datetime
import os
from tkinter import *
...
from src.core.WriteGameSummary import *
from src.core.MyHand20_Analysis import *
from src.core.ShowDownGame import *
from src.core.PlaySixHands20 import *

scenario_description = "Phil Predict > 1400, "
...
global player_names, player_wilds, card_list
overall_start_time = time.time()
NUMBER_OF_CARDS = 25

player_names = ["Peter ", "Johnny", "Ming  ", "Tony  ", "Edmond", "Philip"]
player_cumulative_ante = [0,0,0,0,0,0,0]
player_cumulative_surrender = [0,0,0,0,0,0,0]
player_cumulative_points = [0, 0, 0, 0, 0, 0]
player_cumulative_totals = [0, 0, 0, 0, 0, 0]
player_cumulative_plays = [0, 0, 0, 0, 0, 0]
player_cumulative_wilds = [0, 0, 0, 0, 0, 0]
player_cumulative_wins = [0, 0, 0, 0, 0, 0]
k_num = 0
print (MODE)
...
if MODE == "Batch":
    ...
    for k_num in range (NUMBER_OF_HANDS):
        ...
        # play_six_hands returns player_points and play_hand25 but I want to play through 50 hands per player
        sd = ShowDownGame(play_six_hands.player_points, play_six_hands.play_hand20)

        for i in range (6):
            player_cumulative_ante[i] += sd.player_ante[i]
            player_cumulative_surrender[i] += sd.player_surrender[i]
            player_cumulative_points[i] += play_six_hands.player_points[i]
            player_cumulative_wilds[i] += play_six_hands.player_wilds[i]
            player_totals[i] = sd.player_ante[i] + sd.player_surrender[i] + play_six_hands.player_points[i]
            player_cumulative_totals[i] += player_totals[i]
            player_cumulative_wins[i] += sd.player_wins[i]
            if play_six_hands.play_hand20[i] == True:
                player_cumulative_plays[i] += 1

        print ('  Seed  Short Description  Name  Wilds   H20    Predict  Quality  Signal  Actual   Play   Points  Ante  Surr  Total  Wilds  Ante   Surr  Points  Totals   Plays  Wins  Games')
        for i in range(6):
            print (str(global_variables.random_seed), scenario_description, end="")
            print ('{:>8}'.format(player_names[i]), str(play_six_hands.player_wilds[i]),end="")
            print ('{:>8}'.format(str(play_six_hands.player_score_hand20[i])),end="")
            print ('{:>8}'.format(str(play_six_hands.player_score_prediction[i])),end="")
            print ('{:>9}'.format(str(play_six_hands.play_hand20_quality[i])),end="")
            print ('{:>9}'.format(str(play_six_hands.hand20_signal[i])),end="")
            print ('{:>8}'.format(str(play_six_hands.points_stored[i][0])),end="")
            print ('{:>6}'.format(str(play_six_hands.play_hand20[i])),end="")
            print ('{:>6}'.format(str(play_six_hands.player_points[i])),end="")
            ...
            print ('{:>6}'.format(str(player_cumulative_wilds[i])),end="")
            print ('{:>6}'.format(str(player_cumulative_ante[i])),end="")
            print ('{:>6}'.format(str(player_cumulative_surrender[i])),end="")
            print ('{:>6}'.format(str(player_cumulative_points[i])),end="")
            print ('{:>6}'.format(str(player_cumulative_totals[i])),end="")
            print ('{:>6}'.format(str(player_cumulative_plays[i])),end="")
            print ('{:>6}'.format(str(player_cumulative_wins[i])), '{:>6}'.format(k_num + 1))

        game_string = ""
        for i in range(6):
            game_string = str(global_variables.random_seed) + ', ' + scenario_description
            ...
            game_string += '{:>6}'.format(str(player_totals[i])) + ', '
            game_string += '{:>6}'.format(str(player_cumulative_wilds[i])) + ', '
            game_string += '{:>6}'.format(str(player_cumulative_ante[i])) + ', '
            game_string += '{:>6}'.format(str(player_cumulative_surrender[i])) + ', '
            game_string += '{:>6}'.format(str(player_cumulative_points[i])) + ', '
            game_string += '{:>6}'.format(str(player_cumulative_totals[i])) + ', '
            game_string += '{:>6}'.format(str(player_cumulative_plays[i])) + ', '
            game_string += '{:>6}'.format(str(player_cumulative_wins[i])) + ', ' + '{:>6}'.format(str(k_num + 1)) + ', '
            for j in range(1, 7):
...
def show_next_hand(*args):
    ...
    twentyfive_cards = []
    twentyfive_cards = (sorted(a, cmp=rank_sort, reverse=True))
...
def show_ShowDown():
    play_six_hands = PlaySixHands20(six_hands)
    play_six_hands.play_hand20[0] = True
    player_hand_score = show_player_score()

    for i in range(7):
        print (player_hand_score[i], end="")
    play_six_hands.points_stored[0] = player_hand_score
    sd = ShowDownGame(play_six_hands.points_stored, play_six_hands.play_hand20)
    for i in range(6):
        player_cumulative_ante[i] += sd.player_ante[i]
        player_cumulative_surrender[i] += sd.player_surrender[i]
        player_cumulative_points[i] += play_six_hands.player_points[i]
    ...
    print ('\n  Name  Wilds   H20   Predict Actual    Quality  Play  Points  Ante  Surr  Total  Ante   Surr  Points  Totals   Wins   Games')
    for i in range(6):
        print ('{:>8}'.format(player_names[i]), str(player_wilds[i]), end="")
        print ('{:>8}'.format(str(play_six_hands.player_score_hand20[i])),end="")
        print ('{:>8}'.format(str(play_six_hands.player_score_prediction[i])),end="")
        print ('{:>8}'.format(str(play_six_hands.points_stored[i][0])),end="")
        print ('{:>9}'.format(str(play_six_hands.play_hand20_quality[i])),end="")
...
if MODE == "Interactive":
    ...
    show_next_hand_button = Button(top, text="Show Next Hand", command=show_next_hand).place(x=1080-200,y=225, width=140, height=24)
    show_best25_hand_button = Button(top, text="Show Best Hand", command=show_best25_hand).place(x=1080-200,y=250,width=140, height=24)
    showdown_hand_button = Button(top, text="ShowDownGame", command=show_ShowDown).place(x=1080-200,y=275,width=140, height=24)
