...
lblLine = tk.Label(bottomFrame, text="**********Client List**********").pack()
scrollBar = tk.Scrollbar(bottomFrame)
scrollBar.pack(side=tk.RIGHT, fill=tk.Y)
tkDisplay = tk.Text(bottomFrame, height=15, width=30)
...
def send_receive_client_message(client_connection, client_ip_addr):
    ...
    while True:
        ...
        client_msg = data
        print ("chat rec'd from client:", client_name, client_msg )
        if not data: break
        if data == "exit": break
        ...
        for c in clients:
            if c != client_connection:
                ...
                print ("sending msg to", index, msg)
