...
v9 adds server back - adds a button I'm done that sends player_hand to server
"""

""" pp_client v1 pyramid poker client
v1. works - sends player_hand to server when "I'm Done" is clicked
...
def connect():
    ...
    print ("client function connect started...")
    # if name entry is blank, post error message on tkmessagebox else, save to username and connect_to_server
    if len(entName.get()) < 1:
        tkmessagebox.showerror(title="ERROR!!!", message="You MUST enter your first name <e.g. John>")
    else:
        username = entName.get()
        print ("client - username entered", username)
    #     save_name(username)


def save_name(string_name):
    pass
    # byte_name = string_name.encode('utf-8')
    #
    # client.send(byte_name)  # Send name (byte type) to server after connecting
    #
...
def show_best25_hand(*args):
    ...
    y_list = [0, 500, 400, 300, 200, 100, 0]
    ...
    x_label = 10 * X_GAP
    y_label = 310 + TK_Y_OFFSET # aligns with best_hand
    x = x_label
    y = y_label
...
def show_player_score():
    ...
    if valid_hand == False:
        ...
        print ("Player Hand Invalid, player_score set to -9999")
...
def send_results():
    ...
    client_player_hand = show_player_score()
    done_button["state"] = "disabled"
...
if True:
    ...
    ties = 0
    cumulative_points = 0
    w = tk.Canvas(window, height=7*Y_GAP+10, width=15*X_GAP, background="light blue", relief="raised")
    count = 0

    xloc1 = 11 * X_GAP
    yloc1 = 2 * Y_GAP + 10 + TK_Y_OFFSET
    xloc2 = 13 * X_GAP
