""" pp_client3 builds off of chat_client_gui
v3. add the dealing of hands and deck of cards, receives hand instead of welcome message. Chat function seems to be working
...
def connect_to_server(name):
    ...
    print (client, HOST_PORT, HOST_ADDR)
...
def receive_message_from_server(sck, m):
    ...
    print ("entering receive_message_from_server")
    while True:
        bytes_from_server = sck.recv(8192)
        try:
            from_server = pickle.loads(bytes_from_server)
        ...
        # display_cardlist message from server on the chat window

        # enable the display_cardlist area and insert the text and then disable.
